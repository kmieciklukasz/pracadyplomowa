        ...
        <b-row>
          <b-col sm="6" offset-sm="3">
            <b-form @submit="onSubmit" @reset="onReset">
              ...
              >
              &nbsp;
              <b-button type="reset" variant="danger">Reset</b-button> </b-form
            ><br />
            <router-link to="/panel" tag="span">
              <b-button type="submit" variant="success">Panel</b-button>
            </router-link>
          </b-col>
        </b-row>
      </div>
...
export default {
  ...
  methods: {
    async onSubmit(event) {
      ...
      await this.$store.dispatch("login3", {
        email: this.email,
        password: this.password,
        returnSecureTokej: true,
      });
      this.$router.push({ name: "pracc" });
    },
    onReset(event) {
      event.preventDefault();
      this.email = "";
